...
async function getAllMessagesWithAuthor() {
  const { rows } = await pool.query(
    'SELECT messages_1.*, users.firstname FROM messages_1 JOIN users ON authorid = users.id',
  )
  return rows
}
...
async function createNewMessage(message, authorid) {
  await pool.query(
    'INSERT INTO messages_1 (message, authorid, date) VALUES ($1, $2, $3)',
    [message, authorid, new Date()],
  )
}
